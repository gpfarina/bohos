/* 
 * Copyright (C) Michael Goldweber <mikeyg@cs.xu.edu>
 *
 ...
 *      Modified by Renzo Davoli for Bohos. May 2009 
 */
...
int		p4inc=0;		/* p4 incarnation number */
...
void test() {
    ...
    SYSCALL(CREATETHREAD, (int)&p4state, 1, 0);				/* start p4     */
    
    SYSCALL(CREATETHREAD, (int)&p5state, 2, 0); 			/* start p5     */
    
    SYSCALL(CREATETHREAD, (int)&p6state, 3, 0);				/* start p6		*/
    
    SYSCALL(CREATETHREAD, (int)&p7state, 4, 0);				/* start p7		*/
	...
	creation = SYSCALL(CREATETHREAD, (int)&p8rootstate, 8, 0);
...
/* Terminate here the second incarnation of P4 */
	    ...
	    /* a V(synp4). the new process will send synp4 and terminate.*/
...
void p8root() {
    ...
    SYSCALL(CREATETHREAD, (int)&child1state, 4, 0);
    
    SYSCALL(CREATETHREAD, (int)&child2state, 4, 0);
    ...
    for (grandchild=0; grandchild < NOLEAVES; grandchild++) {
	SYSCALL(VERHOGEN, (int)&blkp8l, 0, 0);
    }
...
void child1() {
    ...
    SYSCALL(CREATETHREAD, (int)&gchild2state, 1, 0);
...
void child2() {
    ...
    SYSCALL(CREATETHREAD, (int)&gchild3state, 2, 0);
    
    SYSCALL(CREATETHREAD, (int)&gchild4state, 3, 0);
...
void p8leaf() {
    ...
    SYSCALL(PASSERN, (int)&blkp8l, 0, 0);
